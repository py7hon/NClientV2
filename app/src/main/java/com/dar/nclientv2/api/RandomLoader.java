...
public class RandomLoader {
    ...
    private void loadRandomGallery(){
        if(galleries.size()>=MAXLOADED)return;
            Global.client.newCall(new Request.Builder().url("https://id.nhent.ai/random").build()).enqueue(new Callback() {
                @Override
                ...
                public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                    Document doc=Jsoup.parse(response.body().byteStream(),"UTF-8","https://id.nhent.ai");
